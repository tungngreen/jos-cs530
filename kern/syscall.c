...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
	...
	//always run at code protection level
	e->env_tf.tf_cs |= 3;
...
sys_env_set_pgfault_upcall(envid_t envid, void *func)
{
	...
	// Set the page fault upcall for 'envid' by modifying the corresponding struct
	// Env's 'env_pgfault_upcall' field. 
	struct Env *env;
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	ret = page_insert(env->env_pml4e, page, va, perm);
	if (ret < 0) {
	 	return -E_BAD_ENV;
	}
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	//	-E_BAD_ENV if srcenvid and/or dstenvid doesn't currently exist,
	//		or the caller doesn't have permission to change one of them.
	int ret = envid2env(srcenvid, &srcenv, 0);
	if (ret < 0) {
		return -E_BAD_ENV;
	}
	...
	//	-E_INVAL is srcva is not mapped in srcenvid's address space.
	pte_t *pte;
	struct PageInfo *page = page_lookup(srcenv->env_pml4e, srcva, &pte);
	if (!page) {
		return -E_INVAL;
	}
	...
	//	-E_NO_MEM if there's no memory to allocate any necessary page tables.
	ret = page_insert(dstenv->env_pml4e, page, dstva, perm);
	if (ret < 0) {
		return -E_NO_MEM;
	}
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
		...
		if ((uintptr_t)srcva < UTOP) {
			...
			//	-E_INVAL if srcva < UTOP but srcva is not mapped in the caller's
			//		address space.
			if ((page = page_lookup(curenv->env_pml4e, srcva, &pte)) == NULL) {
				return -E_INVAL;
			}
			//	-E_INVAL if (perm & PTE_W), but srcva is read-only in the
			//		current environment's address space.
			...
			ret = page_insert(env->env_pml4e, page, env->env_ipc_dstva, perm);
			if (ret < 0) {
				return -E_NO_MEM;
			}
